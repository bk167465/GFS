...
func runChunkServer(serverID string, port string, masterAddr string) error {
	cs := chunkserver.NewChunkServer(serverID)
	server := chunkserver.NewChunkServerGRPC(cs)
	return server.StartServer(serverID, port, masterAddr)
}
...
func ChunkCommand() {
	...
	master := chunkCmd.String("master", "localhost:5000", "Master address")

	chunkCmd.Parse(os.Args[2:])

	log.Printf("Starting Chunk Server: %s...", *serverID)

	if err := runChunkServer(*serverID, *port, *master); err != nil {
